...
try{
 ...
 const fly=new THREE.Group();fly.position.y=.65;scene.add(fly);
 ...
 for(const side of [-1,1]){for(let i=0;i<3;i++){const z=.4-i*.5;segment([side*.3,-.15,z],[side*.85,-.4,z+.25-i*.18]);segment([side*.85,-.4,z+.25-i*.18],[side*1.15,-.65,z+.48-i*.3]);}segment([side*.18,.22,1],[side*.27,.55,1.25],.016);const wing=oval(fly,wingMat,[side*.91,.35,-.54],[.6,.045,1.1]);wing.rotation.y=side*.48;for(let i=0;i<3;i++)segment([side*.25,.36,0],[side*(.65+i*.3),.36,-1.18+i*.12],.006,new THREE.MeshBasicMaterial({color:0x95bfa4,transparent:true,opacity:.38}));}
 const activity=new THREE.MeshStandardMaterial({color:0xc8ef61,emissive:0xc8ef61,emissiveIntensity:.1,transparent:true,opacity:.8});oval(fly,activity,[0,.23,.77],[.11,.1,.1]);
