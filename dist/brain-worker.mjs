...
let brain=null, running=false, timer=null, loading=false;
...
function tick(){
 ...
 try{
  const start=performance.now();const count=brain._fly_step(500);const elapsed=performance.now()-start;
  post('sample',{time:brain._fly_time(),spikes:count,speed:50/elapsed,touched:brain._fly_live_count()});
...
self.onmessage=e=>{
 ...
 if(type==='load')load();
 else if(type==='run'&&brain&&!running){running=true;tick();}
