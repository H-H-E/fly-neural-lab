...
void _before_run_default_synapses_pre_push_spikes()
{
    ...
    ///// CONSTANTS ///////////
    const int64_t _n_sources = 138639;
const int64_t _n_targets = 138639;
const size_t _num_source_dt = 1;
const size_t _num_spikespace = 138640;
double* const _array_default_synapses_delay = _dynamic_array_default_synapses_delay.empty()? 0 : &_dynamic_array_default_synapses_delay[0];
const size_t _numdelay = _dynamic_array_default_synapses_delay.size();
    ///// POINTERS ////////////
        
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_default_neurons__spikespace = _array_default_neurons__spikespace;
    double*   _ptr_array_default_synapses_delay = _array_default_synapses_delay;

    std::vector<double> &real_delays = _dynamic_array_default_synapses_delay;
    double* real_delays_data = real_delays.empty() ? 0 : &(real_delays[0]);
    int32_t* sources = default_synapses_pre.sources.empty() ? 0 : &(default_synapses_pre.sources[0]);
    const size_t n_delays = real_delays.size();
    const size_t n_synapses = default_synapses_pre.sources.size();
    default_synapses_pre.prepare(_n_sources,
                           _n_targets,
