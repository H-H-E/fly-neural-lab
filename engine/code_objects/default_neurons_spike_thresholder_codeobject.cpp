#include "fly_lazy.h"
#include <cstdint>
extern int64_t fly_deadline[138639];
extern int64_t fly_refractory_ticks[138639];
...
const size_t _num_spikespace = 138640;
const size_t _numlastspike = 138639;
const size_t _numnot_refractory = 138639;
const size_t _numt = 1;
const size_t _numv = 138639;
const double v_th = - 0.045;
    ///// POINTERS ////////////
        
    int32_t* __restrict  _ptr_array_default_neurons__spikespace = _array_default_neurons__spikespace;
    ...
    long _count = 0;
    for(size_t pos=0; pos<fly_live.size(); pos++)
    {
        const size_t _idx=fly_live[pos];
        const size_t _vectorisation_idx = _idx;
                
        const char not_refractory = _ptr_array_default_neurons_not_refractory[_idx];
        const double v = _ptr_array_default_neurons_v[_idx];
        char _cond;
        if(!not_refractory)
            _cond = (v > v_th) && false;
        else 
        ...
        if(_cond) {
            _ptr_array_default_neurons__spikespace[_count++] = _idx;
            _ptr_array_default_neurons_not_refractory[_idx] = false;
            _ptr_array_default_neurons_lastspike[_idx] = _ptr_array_defaultclock_t[0];
            fly_deadline[_idx] = _array_defaultclock_timestep[0] + fly_refractory_ticks[_idx];
        }
    }
    _ptr_array_default_neurons__spikespace[N] = _count;
    ...
    default_neurons_spike_thresholder_codeobject_profiling_info += _run_time;
}
