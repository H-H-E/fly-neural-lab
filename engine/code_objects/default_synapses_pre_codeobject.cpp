...
void _run_default_synapses_pre_codeobject()
{
...
double* const _array_default_synapses_w = _dynamic_array_default_synapses_w.empty()? 0 : &_dynamic_array_default_synapses_w[0];
const size_t _numw = _dynamic_array_default_synapses_w.size();
...
const size_t _num_postsynaptic_idx = _dynamic_array_default_synapses__synaptic_post.size();
    ...
    double* __restrict  _ptr_array_default_synapses_w = _array_default_synapses_w;
    ...
    {
    std::vector<int> *_spiking_synapses = default_synapses_pre.peek();
    ...
    
    {
            ...
            _spiking_synapse_idx++)
        {
            ...
            const int32_t _postsynaptic_idx = _ptr_array_default_synapses__synaptic_post[_idx];
            fly_touch(_postsynaptic_idx);
            double g = _ptr_array_default_neurons_g[_postsynaptic_idx];
            const char not_refractory = _ptr_array_default_neurons_not_refractory[_postsynaptic_idx];
            const double w = _ptr_array_default_synapses_w[_idx];
            if(not_refractory)
                g += w;
            _ptr_array_default_neurons_g[_postsynaptic_idx] = g;
    ...
    default_synapses_pre_codeobject_profiling_info += _run_time;
}

void _debugmsg_default_synapses_pre_codeobject()
{
    using namespace brian;
    std::cout << "Number of synapses: " << _dynamic_array_default_synapses__synaptic_pre.size() << endl;
}
