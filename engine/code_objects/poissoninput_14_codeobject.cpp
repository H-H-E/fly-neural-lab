...
namespace {
    ...
    long poissoninput_binomial_14(const int vectorisation_idx)
    {
        long X = 0;
        double px = 0.980000000000000;
        ...
        while (U > px)
        {
            ...
            {
                X = 0;
                px = 0.980000000000000;
                U = _rand(vectorisation_idx);
            } else
            {
                U -= px;
                px = ((1-X+1) * 0.020000000000000 * px)/(X*0.980000000000000);
            }
        }
        return X;
    }
...
void _run_poissoninput_14_codeobject()
{
...
const size_t _num_sub_idx = 1;
    ...
    int32_t*   _ptr_array_default_neurons_subgroup_14__sub_idx_1 = _array_default_neurons_subgroup_14__sub_idx_1;
    ...
    const double _lio_1 = 68.75 * mvolt;
    ...
    for(int _idx=0; _idx<_N; _idx++)
    {
        ...
        const int32_t _sub_idx = _ptr_array_default_neurons_subgroup_14__sub_idx_1[_idx];
        const char not_refractory = _ptr_array_default_neurons_not_refractory[_sub_idx];
        fly_touch(_sub_idx);
        double v = _ptr_array_default_neurons_v[_sub_idx];
        if(not_refractory)
            v += _lio_1 * poissoninput_binomial_14(_vectorisation_idx);
        _ptr_array_default_neurons_v[_sub_idx] = v;
    ...
    poissoninput_14_codeobject_profiling_info += _run_time;
}
